...
local ArbeitnowSDK = {}
ArbeitnowSDK.__index = ArbeitnowSDK
...
ArbeitnowSDK._make_feature = _make_feature


function ArbeitnowSDK.new(options)
  local self = setmetatable({}, ArbeitnowSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ArbeitnowSDK:options_map()
...
function ArbeitnowSDK:get_utility()
  return Utility.copy(self._utility)
end


function ArbeitnowSDK:get_root_ctx()
  return self._rootctx
end


function ArbeitnowSDK:prepare(fetchargs)
...
function ArbeitnowSDK:direct(fetchargs)
...
-- by passing it.
function ArbeitnowSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ArbeitnowSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ArbeitnowSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ArbeitnowSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ArbeitnowSDK.new(sdkopts)
  sdk.mode = "test"
