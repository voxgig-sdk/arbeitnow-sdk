...
require_relative 'Arbeitnow_types'
...
class ArbeitnowSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ArbeitnowHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ArbeitnowHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ArbeitnowFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ArbeitnowUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ArbeitnowHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ArbeitnowHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ArbeitnowHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ArbeitnowError.new(
        "#{op}_allow",
        "ArbeitnowSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ArbeitnowHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ArbeitnowError.new(
        "graphql_error", "ArbeitnowSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
