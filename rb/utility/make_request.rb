...
module ArbeitnowUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ArbeitnowResponse.new({})
    ...
    if fetch_err
      ...
      response = ArbeitnowResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ArbeitnowResponse.new(fetched)
    else
