...
class JobEntity
  ...
  def data_set(args)
    if args
      @_data = ArbeitnowHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ArbeitnowHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Job>, Array] the matching Job items; raises ArbeitnowError on failure
