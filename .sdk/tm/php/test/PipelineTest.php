...
{
    private static function utility(): ArbeitnowUtility
    {
        return new ArbeitnowUtility();
    }
    ...
    private static function ctx(array $over = []): ArbeitnowContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ArbeitnowContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ArbeitnowOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ArbeitnowError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ArbeitnowOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ArbeitnowMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ArbeitnowOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ArbeitnowMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ArbeitnowOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ArbeitnowMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArbeitnowSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ArbeitnowMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ArbeitnowResponse([]),
            'result' => new ArbeitnowResult([]),
        ]);
        [, $err] = ArbeitnowMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ArbeitnowSpec([]),
            'response' => null,
            'result' => new ArbeitnowResult([]),
        ]);
        [, $err] = ArbeitnowMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ArbeitnowSpec(['step' => 's']),
            'response' => new ArbeitnowResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ArbeitnowResult([]),
        ]);
        [, $err] = ArbeitnowMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ArbeitnowSpec(['step' => 's']),
            'response' => new ArbeitnowResponse(self::resp(200, ['v' => 1])),
            'result' => new ArbeitnowResult([]),
        ]);
        [, $err] = ArbeitnowMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ArbeitnowSpec(['step' => 's']),
            'response' => new ArbeitnowResponse(self::resp(200, ['v' => 2])),
            'result' => new ArbeitnowResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ArbeitnowMakeResponse::call($ctx);
    ...
    {
        $preset = new ArbeitnowResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ArbeitnowSpec([]),
            'response' => new ArbeitnowResponse([]),
            'result' => new ArbeitnowResult([]),
        ...
        [$got, $err] = ArbeitnowMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ArbeitnowResult([])]);
        [, $err] = ArbeitnowMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ArbeitnowSpec([]), 'result' => null]);
        [, $err] = ArbeitnowMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArbeitnowSpec(['step' => 's']),
            'result' => new ArbeitnowResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ArbeitnowOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArbeitnowMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ArbeitnowSpec(['step' => 's']),
            'result' => new ArbeitnowResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ArbeitnowOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ArbeitnowMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ArbeitnowResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ArbeitnowSpec([]),
            'result' => new ArbeitnowResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ArbeitnowMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArbeitnowMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ArbeitnowError('boom', 'boom');
        $utility->fetcher = function (ArbeitnowContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArbeitnowSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ArbeitnowMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ArbeitnowContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ArbeitnowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ArbeitnowResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ArbeitnowContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ArbeitnowSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ArbeitnowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ArbeitnowResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ArbeitnowSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ArbeitnowMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ArbeitnowMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ArbeitnowMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ArbeitnowResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ArbeitnowDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ArbeitnowResult(['ok' => false])]);
        $this->expectException(ArbeitnowError::class);
        ArbeitnowDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ArbeitnowResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ArbeitnowMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ArbeitnowMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ArbeitnowBaseFeature();
        $b = new ArbeitnowBaseFeature();
        ArbeitnowFeatureAdd::call($ctx, $a);
        ArbeitnowFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ArbeitnowBaseFeature
    {
        $f = new ArbeitnowBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ArbeitnowFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ArbeitnowContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ArbeitnowSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ArbeitnowPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ArbeitnowResponse(['headers' => null]),
            'result' => new ArbeitnowResult([]),
        ]);
        ArbeitnowResultHeaders::call($ctx);
