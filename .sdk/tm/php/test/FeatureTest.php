...
class FtCtrl extends ArbeitnowControl
{
...
class FtClient
{
    ...
    private ArbeitnowUtility $utility;

    public function __construct(ArbeitnowUtility $utility, array $options)
    {
    ...
    public function get_utility(): ArbeitnowUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ArbeitnowContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ArbeitnowContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ArbeitnowContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ArbeitnowConfig::make_feature($name);
    ...
    private static function buildUrl(ArbeitnowSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ArbeitnowError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ArbeitnowResponse($fetched);
        }
    ...
    private function fail(ArbeitnowContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ArbeitnowContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
