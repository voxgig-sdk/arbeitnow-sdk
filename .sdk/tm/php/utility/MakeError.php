...
class ArbeitnowMakeError
{
    public static function call(?ArbeitnowContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ArbeitnowContext([], null);
        }
        $op = $ctx->op ?? new ArbeitnowOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ArbeitnowResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ArbeitnowError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ArbeitnowError('', $msg, $ctx);
        ...
        if ($err instanceof ArbeitnowError) {
