...
MCP server exposing the Arbeitnow SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `arbeitnow_list` | `entity`, optional `query` map | First page of records as JSON |
| `arbeitnow_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `arbeitnow_list` and
`arbeitnow_load` tools become available in new sessions.
