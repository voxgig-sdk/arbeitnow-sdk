...
[MCP](https://modelcontextprotocol.io) server exposing the Arbeitnow SDK as
two agent tools — `arbeitnow_list` and `arbeitnow_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/arbeitnow-mcp)
make build

# 2. Provide credentials via the environment
export ARBEITNOW_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user arbeitnow \
  -- /absolute/path/to/arbeitnow-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./arbeitnow-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// arbeitnow_list: first page of records
{ "entity": "job" }
{ "entity": "job", "query": { } }

// arbeitnow_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/arbeitnow-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user arbeitnow \
     -- "$PWD"/dist/*/arbeitnow-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `arbeitnow_list` and `arbeitnow_load` tools now appear
   in new sessions. Ask the agent to *"list job using arbeitnow"*
   and it calls `arbeitnow_list` with `{"entity":"job"}`.
...
```sh
export ARBEITNOW_APIKEY=sk_live_xxx            # API key
export ARBEITNOW_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `arbeitnow_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `arbeitnow_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ARBEITNOW_APIKEY` | API key sent with every request. |
| `ARBEITNOW_BASE` | Optional override of the API base URL. |
...
```sh
./arbeitnow-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"arbeitnow_load","arguments":{"entity":"job","query":{"id":1}}}}'
```
