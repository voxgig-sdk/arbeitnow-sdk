...
//     (\`new ArbeitnowSDK(...)\` / \`ArbeitnowSDK.test(...)\` become
//     \`ArbeitnowSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new ArbeitnowSDK(...)\` and any
// \`ArbeitnowSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ArbeitnowSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ArbeitnowSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
