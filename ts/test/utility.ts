...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.ARBEITNOW_TEST_LIVE ||
    'TRUE' === process.env.ARBEITNOW_TEST_OVERRIDE
  ) {
  ...
  m.ARBEITNOW_TEST_EXPLAIN = process.env.ARBEITNOW_TEST_EXPLAIN || m.ARBEITNOW_TEST_EXPLAIN
