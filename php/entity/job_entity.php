...
class JobEntity
{
    ...
    {
        if ($args) {
            $this->_data = ArbeitnowHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = ArbeitnowHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws ArbeitnowError on failure (item-5 convention).
     */
