...
from arbeitnow_sdk.core.helpers import get_ctx_prop, to_map
...
class ArbeitnowContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, ArbeitnowSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, ArbeitnowResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, ArbeitnowResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return ArbeitnowError(code, msg, self)
