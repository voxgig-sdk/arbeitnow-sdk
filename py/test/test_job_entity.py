...
class TestJobEntity:
    ...
    def test_should_stream(self):
        ...
        base = ArbeitnowSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ARBEITNOW_TEST_JOB_ENTID JSON to run live")
        client = setup["client"]
...
def _job_basic_setup(extra):
    ...
    client = ArbeitnowSDK.test(options, extra)
    ...
    idmap_resolved = helpers.to_map(
        env.get("ARBEITNOW_TEST_JOB_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("ARBEITNOW_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = ArbeitnowSDK(helpers.to_map(merged_opts))

    _live = env.get("ARBEITNOW_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ARBEITNOW_TEST_EXPLAIN") == "TRUE",
        "live": _live,
