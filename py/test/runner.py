...
class ArbeitnowTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ArbeitnowTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ArbeitnowTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ArbeitnowTestRunner.getenv("ARBEITNOW_TEST_LIVE")
        override = ArbeitnowTestRunner.getenv("ARBEITNOW_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ArbeitnowTestRunner.getenv(key)
        ...
        explain = ArbeitnowTestRunner.getenv("ARBEITNOW_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["ARBEITNOW_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ArbeitnowTestRunner._test_control is not None:
            return ArbeitnowTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ArbeitnowTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ArbeitnowTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ArbeitnowTestRunner.load_test_control()
...
def load_env_local():
    ArbeitnowTestRunner.load_env_local()


def env_override(m):
    return ArbeitnowTestRunner.env_override(m)


def entity_data(v):
    return ArbeitnowTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ArbeitnowTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ArbeitnowTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ArbeitnowTestRunner.load_test_control()


def live_delay_ms():
    return ArbeitnowTestRunner.live_delay_ms()
