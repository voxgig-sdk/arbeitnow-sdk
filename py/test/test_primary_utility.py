...
from arbeitnow_sdk.utility.voxgig_struct import voxgig_struct as vs
...
from arbeitnow_sdk.core.response import ArbeitnowResponse
from arbeitnow_sdk.core.operation import ArbeitnowOperation
from arbeitnow_sdk.core.error import ArbeitnowError
from arbeitnow_sdk.core import helpers
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return ArbeitnowError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = ArbeitnowSDK.test(None, None)
        ...
        class TestInitFeature(ArbeitnowBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(ArbeitnowBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = ArbeitnowSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = ArbeitnowSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = ArbeitnowSDK.test(None, setup_opts)
