// Command Arbeitnow-cli is a CLI + REPL for the Arbeitnow SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      ARBEITNOW_APIKEY (api key), ARBEITNOW_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: ARBEITNOW_APIKEY carries the API key and
	// ARBEITNOW_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ARBEITNOW_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ARBEITNOW_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewArbeitnowSDK(opts)
