...
boru-driven command-line client **and** interactive REPL for the Arbeitnow
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/arbeitnow-cli)
make build
...
./arbeitnow-cli list job

# 5. Override the API base URL for a single call
ARBEITNOW_BASE=https://api.example.com ./arbeitnow-cli list job
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/arbeitnow-cli
   ```
...
Run with no arguments to open a REPL (prompt `arbeitnow>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/arbeitnow-cli`. |
...
(`./arbeitnow-cli <expr>`) and interactively (the REPL), and expressions compose the
