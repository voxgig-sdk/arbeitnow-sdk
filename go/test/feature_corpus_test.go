...
//
// NewArbeitnowSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ArbeitnowSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewArbeitnowSDK(opts)
}
...
func fcCandidates(client *sdk.ArbeitnowSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ArbeitnowSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ArbeitnowSDK, name string) any {
