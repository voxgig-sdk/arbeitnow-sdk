...
func TestJobEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ARBEITNOW_TEST_JOB_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func jobBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ARBEITNOW_TEST_JOB_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ARBEITNOW_TEST_JOB_ENTID"])
	...
	if env["ARBEITNOW_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewArbeitnowSDK(core.ToMapAny(mergedOpts))
	}
